...
| `dose_g` | Yes | 5-18 g. |
| `grind` | Yes | 35-75; larger is coarser. Pre-ground/no-grind mode is intentionally disabled. |
| `ratio` | Yes | Sum of machine pours divided by dose. For flash brew this is the hot ratio. |
| `water_ml` | Yes | Hot total for `hot`; hot plus ice for `flash-brew`. |
| `hot_water_ml` | Recommended/Yes | Sum of pours; required for `flash-brew`. |
...
For a hot recipe, sum of pours, `hot_water_ml`, and `water_ml` must match. The hot ratio must be
1:12 through 1:20. Total machine water must be 60-360 ml.

For a flash brew, sum of pours must equal `hot_water_ml`; `water_ml` must equal
`hot_water_ml + ice_g`. Hot ratio must be 1:8 through 1:14 and final ratio must be 1:12 through
1:20.
